...
$results = $c->query("SELECT *, SQRT(POW(69.1 * (latitude - " . $latitude . "), 2) + POW(69.1 * (" . $longitude . " - longitude) * COS(latitude / 57.3), 2)) AS distance FROM jobs HAVING distance < 25 ORDER BY distance LIMIT " . $start . "," . $length);
$jobs = [];
